...
public sealed class GoAffProEventDetectorTests
{
    ...
    public async Task NewOrdersAsync_TracksLastPollTimeAsync()
    {
                ...
                )
                {
                    ...
                    return ordersCallCount == 1
                        ? TestHttpMessageHandler.JsonResponse(
                            """{"orders":[{"id":"o-1"}],"count":1,"limit":100,"offset":0}"""
                        )
                        : TestHttpMessageHandler.JsonResponse(
                            """{"orders":[],"count":0,"limit":100,"offset":0}"""
                        );
                }
        ...
        using var client = new GoAffProClient(
            ...
            {
                BaseUrl = new Uri("https://example.test/v1/", UriKind.Absolute),
            }
        );

        IReadOnlyList<UserOrderFeedItem> firstBatch = await TakeAsync(
            client.NewOrdersAsync(pollingInterval: TimeSpan.FromMilliseconds(5), pageSize: 100),
            expectedCount: 1
        );
        await Task.Delay(10);
        IReadOnlyList<UserOrderFeedItem> secondBatch = await TakeAsync(
            client.NewOrdersAsync(pollingInterval: TimeSpan.FromMilliseconds(5), pageSize: 100),
            expectedCount: 1
        );
    ...
    public async Task NewAffiliatesAsync_TracksLastPollTimeAsync()
    {
            ...
            (request, _) =>
            {
                ...
                )
                {
                    ...
                    return trafficCallCount == 1
                        ? TestHttpMessageHandler.JsonResponse(
                            """{"traffic":[{"affiliate_id":"a-1"}],"count":1,"limit":100,"offset":0}"""
                        )
                        : TestHttpMessageHandler.JsonResponse(
                            """{"traffic":[],"count":0,"limit":100,"offset":0}"""
                        );
                }
        ...
        IReadOnlyList<UserTrafficFeedItem> events = await TakeAsync(
            client.NewTrafficAsync(pollingInterval: TimeSpan.FromMilliseconds(5), pageSize: 100),
            expectedCount: 1
        );
    ...
    public async Task NewOrdersAsync_WithStartTime_UsesStartTimeForFirstPollAsync()
    {
        ...
        using var handler = new TestHttpMessageHandler(
            (request, _) =>
            {
                ...
                {
                    startTimeUsed = request.RequestUri.Query.Contains(
                        "created_at_min=",
    ...
    public async Task NewOrdersAsync_SendsRequiredFieldsQueryAsync()
    {
        ...
        Assert.Contains(
            "fields=id,number,total,subtotal,line_items,commission,created_at,currency,site_id,sub_id,conversion_details",
            query
        );
    }
    ...
    public async Task NewPayoutsAsync_TracksLastPollTimeAsync()
    {
            ...
            (request, _) =>
            {
                ...
                )
                {
                    ...
                    return payoutsCallCount == 1
                        ? TestHttpMessageHandler.JsonResponse(
                            """{"payouts":[{"id":"p-1","created_at":"2026-01-01T00:00:00Z"}],"count":1,"limit":100,"offset":0}"""
                        )
                        : TestHttpMessageHandler.JsonResponse(
                            """{"payouts":[],"count":0,"limit":100,"offset":0}"""
                        );
                }
        ...
        IReadOnlyList<UserPayoutFeedItem> firstBatch = await TakeAsync(
            client.NewPayoutsAsync(pollingInterval: TimeSpan.FromMilliseconds(5), pageSize: 100),
            expectedCount: 1
        );
        await Task.Delay(10);
        IReadOnlyList<UserPayoutFeedItem> secondBatch = await TakeAsync(
            client.NewPayoutsAsync(pollingInterval: TimeSpan.FromMilliseconds(5), pageSize: 100),
            expectedCount: 1
        );
            ...
            (request, _) =>
            {
                ...
                )
                {
                    ...
                    return productsCallCount == 1
                        ? TestHttpMessageHandler.JsonResponse(
                            """{"products":[{"id":1},{"id":2}],"count":2,"limit":100,"offset":0}"""
                        )
                        : TestHttpMessageHandler.JsonResponse(
                            """{"products":[{"id":2},{"id":3}],"count":2,"limit":100,"offset":0}"""
                        );
                }
            ...
            (request, _) =>
            {
                ...
                )
                {
                    ...
                    return transactionsCallCount == 1
                        ? TestHttpMessageHandler.JsonResponse(
                            """{"transactions":[{"tx_id":10}],"count":1,"limit":100,"offset":0}"""
                        )
                        : TestHttpMessageHandler.JsonResponse(
                            """{"transactions":[{"tx_id":11}],"count":1,"limit":100,"offset":0}"""
                        );
                }
    ...
    public async Task StartEventObserverAsync_WhenNewEventsAreDetected_RaisesEventHandlersAsync()
    {
        ...
        client.OrderDetected += (_, args) =>
            orderIds.Add(args.Order.Id?.String ?? args.Order.OrderId?.String ?? string.Empty);
        client.TrafficDetected += (_, args) =>
            affiliateIds.Add(
                args.Traffic.AffiliateId?.String ?? args.Traffic.Id?.String ?? string.Empty
            );
        client.PayoutDetected += (_, args) =>
            payoutIds.Add(args.Payout.Id?.String ?? args.Payout.PayoutId?.String ?? string.Empty);
        ...
        while (orderIds.Count == 0 || affiliateIds.Count == 0 || payoutIds.Count == 0)
        {
            await Task.Delay(10);
