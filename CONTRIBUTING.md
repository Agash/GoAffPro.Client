# Contributing to GoAffPro.Client
...
```bash
dotnet restore GoAffPro.Client.slnx
dotnet build GoAffPro.Client.slnx
...
```bash
dotnet test GoAffPro.Client.slnx --filter "TestCategory!=Integration"
```
